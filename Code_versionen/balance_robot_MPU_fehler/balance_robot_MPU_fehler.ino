...
/*
  Änderung:
  readAll() gibt jetzt bool zurück.

  true  = 14 Bytes wurden erfolgreich gelesen
  false = MPU/I2C-Lesefehler

  Dadurch kann der Code verhindern, dass mit fehlerhaften Sensordaten
  weitergeregelt wird.
*/
bool readAll(float &ax, float &ay, float &az,
  ...
  int bytesRead = Wire.requestFrom(MPU_ADDR, 14);
  if (bytesRead != 14) {
...
void calibrateGyro() {
  ...
  while (validSamples < CALIB_SAMPLES) {
    ...
    if (readAll(ax, ay, az, gx, gy, gz)) {
      ...
      i2cErrorCount++;
      Serial.println("I2C Fehler während Gyro-Kalibrierung");
    }
...
void setup() {
  ...
  while (!readAll(ax, ay, az, gx, gy, gz)) {
    i2cErrorCount++;
    stopMotors();
    Serial.println("I2C Fehler beim Lesen des Startwinkels");
    delay(100);
  }
...
void loop() {
  ...
  if (!readAll(ax, ay, az, gx, gy, gz)) {
    ...
    if (millis() - lastSerialMillis >= 100) {
      ...
      Serial.print("I2C/MPU Lesefehler, Motoren gestoppt. Fehleranzahl=");
